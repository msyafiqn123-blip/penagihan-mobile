...
import { LayoutDashboard, Users, UploadCloud, KeyRound, LogOut, Menu, X, Sun, FileText } from 'lucide-react';
...
export default function AdminLayout({
  ...
  const navItems = [
    ...
    { name: 'Data Piutang 2026', path: '/admin/detail-nop', icon: FileText },
    { name: 'Capaian', path: '/admin/capaian', icon: LayoutDashboard }, // using LayoutDashboard as Trophy equivalent
    { name: 'Manajemen Akun', path: '/admin/accounts', icon: Users },
    { name: 'Update Lunas', path: '/admin/update-lunas', icon: UploadCloud },
    { name: 'Update DHKP', path: '/admin/update-dhkp', icon: UploadCloud },
    { name: 'Update Password', path: '/admin/update-password', icon: KeyRound },
  ];
    ...
    <>
      ...
      <div className="p-4 border-t border-slate-100">
        <div className="flex items-center p-3 bg-slate-50 border border-slate-100 rounded-xl mb-4">
          <div className="w-10 h-10 rounded-full bg-blue-500 text-white flex items-center justify-center font-bold text-lg mr-3 shadow-sm flex-shrink-0">
            A
          </div>
          <div className="overflow-hidden">
            <p className="font-bold text-sm text-slate-800 truncate">admin</p>
            <p className="text-[10px] text-slate-500 font-semibold tracking-wider uppercase mt-0.5 truncate">ADMINISTRATOR</p>
          </div>
        </div>
    ...
    <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        ...
        <header className="md:hidden flex items-center justify-between p-4 bg-white border-b border-slate-200 flex-shrink-0 z-10">
          <h1 className="text-lg font-bold tracking-tight mb-0 flex items-center">
            <span className="text-blue-600">Penagihan</span> <span className="text-orange-500 ml-1">Mobile</span>
          </h1>
