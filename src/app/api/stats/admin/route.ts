...
export async function GET(request: Request) {
  try {
    ...
    const kecMap: Record<string, { lunas: number, belum: number, total: number }> = {};
    kecStatsRaw.forEach(r => {
      const kec = r.nm_kecamatan;
      if (!kecMap[kec]) kecMap[kec] = { lunas: 0, belum: 0, total: 0 };
      
      const count = r._count._all;
      kecMap[kec].total += count;
      if (r.status_pembayaran_sppt === 'LUNAS') kecMap[kec].lunas += count;
      else kecMap[kec].belum += count;
    });
    ...
    const kelMap: Record<string, { kec: string, lunas: number, belum: number, total: number }> = {};
    kelStatsRaw.forEach(r => {
      const kel = r.nm_kelurahan;
      if (!kelMap[kel]) kelMap[kel] = { kec: r.nm_kecamatan, lunas: 0, belum: 0, total: 0 };
      
      const count = r._count._all;
      kelMap[kel].total += count;
      if (r.status_pembayaran_sppt === 'LUNAS') kelMap[kel].lunas += count;
      else kelMap[kel].belum += count;
    });
    ...
    return NextResponse.json({
      overall: { totalCount, totalLunas, totalBelum },
      kecamatanStats,
      kelurahanStats
    });
